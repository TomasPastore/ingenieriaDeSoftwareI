...
class Portfolio < SummarizingAccount
  ...
  def add_account(account)
    raise self.class.ACCOUNT_ALREADY_MANAGED if self.manages(account) or account.manages(self)
    @accounts << account
  end
  ...
  def visit_accounts_with(visitor)
    @accounts.each { | account | account.accept(visitor)}
  end
end